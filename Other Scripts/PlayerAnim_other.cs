...
public class PlayerAnim_other : MonoBehaviour
{
    Rigidbody2D rb;
    Animator anim;
    ...
    void Start() 
    {
        ...
        movement = GetComponent<PlayerMov_other>(); //为了让两个代码可以互相访问 要获取另一个代码（代码也是一个组件）
    ...
    void AnimControl() //动画播放方法
    {
        ...
        anim.SetBool(isFallID, movement.isFall);
    ...
    private void OnCollisionEnter2D(Collision2D collision) //与敌人碰撞 写在了动画脚本里有点乱 能力不到位 后期有能力后调整
    {
        ...
        if (collision.gameObject.tag == "Enemy") //如果碰撞的tag为Enemy
        {
            ...
            if (anim.GetBool(isFallID)) //如果是跳跃的动画
            {
